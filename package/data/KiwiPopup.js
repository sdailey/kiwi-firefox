...
(function() {
  var Conversations, Credits, CustomSearch, FixedView, KiwiSlice, Loading, SwitchView, UserPreferences, View, Widget, algoliaPHclient, algoliaPHindex, bindGoToViewButtons, capitalizeFirstLetter, fetchAlgoliaProductHuntResultsFirst, fixedViews, formatTime, getRandom, getURLParam, htmlEntities, initAlgoliaPHcustomSearch, initialize, moveArrayElement, preferencesOnlyPage, receiveParcel, renderExtensionHeight, renderedBool, sendParcel, switchViews, tailorRedditAndHNresults_returnHtml, tailorResults, viewElementId, _tailorHNcomment,
    ...
    CustomSearch.prototype.__renderStates__ = function() {
      return {
        ...
        opened: {
          paint: (function(_this) {
            return function(popupParcel) {
              var activeClass, ariaPressedState, brandedTitle, customSearchResultsHTML, duplicateFixedHeight, index, openedCustomSearchHTML, queryString, resultsSummaryArray, serviceDisabledAttr, serviceInfoObject, service_PreppedResults, tagActiveChecked, tagDisabledAttr, tagName, tagObject, _i, _j, _len, _len1, _ref, _ref1, _ref2;
          ...
          bind: (function(_this) {
            return function(popupParcel) {
              ...
              jumpToServiceCustomResults.bind('click', function(ev) {
                ...
                pxFromTop = parseInt($($(_this.DOMselector + " #customSearchResults .serviceResultJumpTo")[serviceIndex]).offset().top);
                return $('body,html').scrollTop(pxFromTop - 100);
              });
          ...
          bind: (function(_this) {
            return function(popupParcel) {
              ...
              jumpToService.bind('click', function(ev) {
                var pxFromTop, serviceIndex;
                serviceIndex = parseInt($(ev.target).data('serviceindex'));
                pxFromTop = parseInt($($(_this.DOMselector + " #resultsByService .serviceResultJumpTo")[serviceIndex]).offset().top);
                return $('body,html').scrollTop(pxFromTop - 100);
              });
  ...
  initAlgoliaPHcustomSearch = function() {
    algoliaPHclient = algoliasearch('0H4SMABBSG', '9670d2d619b9d07859448d7628eea5f3');
    return algoliaPHindex = algoliaPHclient.initIndex('Post_production');
  };

  initAlgoliaPHcustomSearch();

  fetchAlgoliaProductHuntResultsFirst = function(parcel) {
    if (algoliaPHindex != null) {
      return algoliaPHindex.search(parcel.customSearchRequest.queryString, function(err, content) {
        if (err) {
          console.error(err);
        ...
        return self.port.emit('kiwiPP_post_customSearch', parcel);
      });
    } else {
      parcel.customSearchRequest.servicesToSearch.productHunt.rawResults = [];
      return self.port.emit('kiwiPP_post_customSearch', parcel);
  ...
  sendParcel = function(parcel) {
    ...
    switch (parcel.msg) {
      case 'kiwiPP_refreshSearchQuery':
        return self.port.emit('kiwiPP_refreshSearchQuery', parcel);
      case 'kiwiPP_post_customSearch':
        if (parcel.customSearchRequest.servicesToSearch.productHunt != null) {
          return fetchAlgoliaProductHuntResultsFirst(parcel);
        } else {
          return port.postMessage(parcel);
        }
        break;
      case 'kiwiPP_request_popupParcel':
        return self.port.emit('kiwiPP_request_popupParcel', parcel);
      case 'kiwiPP_post_savePopupParcel':
        return self.port.emit('kiwiPP_post_savePopupParcel', parcel);
      case 'kiwiPP_post_save_a_la_carte':
        return self.port.emit('kiwiPP_post_save_a_la_carte', parcel);
      case 'kiwiPP_clearAllURLresults':
        return self.port.emit('kiwiPP_clearAllURLresults', parcel);
      case 'kiwiPP_refreshURLresults':
        return self.port.emit('kiwiPP_refreshURLresults', parcel);
      case 'kiwiPP_researchUrlOverrideButton':
        return self.port.emit('kiwiPP_researchUrlOverrideButton', parcel);
    }
  };
  ...
  self.port.on('kiwi_fromBackgroundToPopup', function(parcel) {
    return receiveParcel(parcel);
  });
